...
class log_handler():
        def __init__(self, args) -> None:
                ...
                self.validation_log = os.path.join(self.save_dir_path, f"validation({str(args.seed).zfill(2)}_{args.train_ratio})_{datetime.now().strftime('%y%m%d-%H%M%S-%f')}.log")
                self.test_log = os.path.join(self.save_dir_path, f"test({str(args.seed).zfill(2)}_{args.train_ratio}_{datetime.now().strftime('%y%m%d-%H%M%S-%f')}).log")
